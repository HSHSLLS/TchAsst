...
public class TitleItemDecoration extends RecyclerView.ItemDecoration {
    ...
    public TitleItemDecoration(Context context, List<String> datas) {
        ...
        mPaint.setAntiAlias(true); //抗锯齿，使图像边缘相对清晰
...
/*
        ...
        mPaint.getTextBounds(mDatas.get(position), 0, mDatas.get(position).length(), mBounds); //测量出即将绘制的text的矩形mBounds
        c.drawText(mDatas.get(position), child.getPaddingLeft(), child.getTop() - params.topMargin - (mTitleHeight / 2 - mBounds.height() / 2), mPaint); //参数三：float y :The y-coordinate of the baseline of the text being drawn 即文字的底边
    }
